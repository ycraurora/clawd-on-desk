...
2. OpenAI 官方现在是否支持 Windows 上的 Codex hooks。
3. Clawd 当前是否能自动感知“跑在 WSL 里的 Codex 会话”。
...
- OpenAI 官方当前仍写明：Codex hooks 在 Windows 上暂时禁用。
- Clawd 当前对 Codex 的集成方式是轮询 `~/.codex/sessions` JSONL 日志，而不是使用 hooks。
- 当 Clawd 跑在 Windows、Codex 跑在 WSL 且仍使用 Linux 默认 home 时，Clawd 默认不会扫到 WSL 里的 `/home/<user>/.codex/sessions`。
- 所以“Codex 不支持 WSL”这个说法不准确。更准确的说法是：“Codex 官方支持 WSL2，但 Clawd 目前没有把 Codex on WSL 的支持边界在文档里讲清楚，默认也不会自动发现独立 Linux home 下的 Codex 会话。”
...
### 3. Windows 上的 Codex hooks 目前仍禁用

OpenAI 官方 Hooks 文档当前明确写着：

- Hooks 仍处于 experimental 状态
...
这件事非常重要，因为它解释了为什么 Clawd 没有在 Windows 上把 Codex 做成和 Claude Code 类似的 hook 集成。
...
### 1. Clawd 对 Codex 用的是 JSONL 轮询，不是 hooks
...
对应的监控实现写在 [`agents/codex-log-monitor.js`](../../agents/codex-log-monitor.js)。

这个实现会把 `~` 展开成当前进程自己的 `os.homedir()`。也就是说：
...
### 2. Clawd 当前没有单独的“Codex on WSL”自动发现逻辑

当前主进程在 [`src/main.js`](../../src/main.js) 里直接按默认配置启动 Codex 日志监控器，没有看到面向用户的 WSL session 目录配置入口，也没有专门把 `\\wsl$\...` 路径纳入默认扫描。

这意味着：

- `Codex native Windows + Clawd on Windows`：开箱即用
- `Codex in WSL2 + Clawd on Windows + Linux 默认 home`：默认不通
- `Codex in WSL2 + 共享到 Windows 的 .codex`：按官方 `CODEX_HOME` 方案推导，Clawd 才有机会扫到同一份 session 目录

最后这一条是根据 OpenAI 官方的 `CODEX_HOME` 共享方案和 Clawd 当前的轮询路径推导出的结论。

### 3. Clawd 仓库里有远程/旁路方案，但不是 WSL 开箱即用方案

仓库里已有 [`hooks/codex-remote-monitor.js`](../../hooks/codex-remote-monitor.js)，它会在另一端轮询 `~/.codex/sessions`，再把状态 POST 回本地 Clawd。
...
截至 2026-04-19，更准确的结论是：

1. OpenAI 官方支持 Codex 跑在 WSL2 里。
2. OpenAI 官方当前仍写明 Codex hooks 在 Windows 上禁用。
3. Clawd 因此在 Windows 上对 Codex 采用 JSONL 日志轮询，而不是 hooks。
4. Clawd 当前默认轮询的是宿主机自己的 `~/.codex/sessions`。
5. 所以当 Codex 运行在 WSL2 且仍使用 Linux 默认 `~/.codex` 时，Clawd 默认不会自动发现这些会话。
...
> OpenAI 官方是支持 Codex 跑在 WSL2 里的；但截至 2026-04-19，官方 Hooks 文档仍写明 Windows 上的 Codex hooks 暂时禁用。Clawd 现在对 Codex 走的是 `~/.codex/sessions` 日志轮询，所以当 Clawd 跑在 Windows、Codex 跑在 WSL 且使用 Linux 默认 home 时，Clawd 默认不会自动扫到那份 session 日志。当前问题更像是 Clawd 文档和集成边界没有讲清楚，而不是 Codex 官方不支持 WSL。
...
2. 参考仓库现有的 [`hooks/codex-remote-monitor.js`](../../hooks/codex-remote-monitor.js) 思路，把 WSL 视作“另一端”，主动把状态回传给 Windows 上的 Clawd。
3. 修改 Clawd 自己的 Codex sessionDir 解析逻辑，显式支持 `\\wsl$\...` 路径。

其中：

- 第 1 条是 OpenAI 官方给出的共享方案。
- 第 2 条是仓库里已经存在的旁路思路。
- 第 3 条属于 Clawd 侧新增功能，不是当前默认行为。
